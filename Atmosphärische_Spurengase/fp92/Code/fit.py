...
def bw(x,par):
    ...
    Gamma = par[2]
    try: 
        gam = math.sqrt(M**2 * (M**2 + Gamma**2))
        k = (2 * math.sqrt(2) * M * Gamma * gam) / (math.pi * math.sqrt(M**2 + gam)) 
...
tmpHist = rootfile.Get("mass_l_11") 
...
fConv.SetParameter(1,90)
fConv.SetParameter(2,4)
# fConv.SetParameter(3,)
# fConv.SetParameter(4,90)
fConv.SetParameter(5,4)


fConv.FixParameter(3,1)# this would be the normalization of the gauss
fConv.FixParameter(4,0) # this would be the mean of the gauss
# fConv.FixParameter(0,250.0) # this would be the normalization of the gauss
# fConv.FixParameter(1,90.0) # this would be the mean of the gauss

fConv.SetLineColor(ROOT.kGreen)
# fBw.SetNpx(1000) # sets the amount of sampling points in x range. Do not choose too small for convolution later on
...
tex.DrawLatex(0.69, 0.85, "Gauss Fit")
tex.DrawLatex(0.69, 0.80, "M_{Z} = %.3f #pm %.3f" %  (fGauss.GetParameter(1), fGauss.GetParError(1)))
tex.DrawLatex(0.69, 0.75, "#sigma = %.3f #pm %.3f" % (fGauss.GetParameter(2), fGauss.GetParError(2)))
tex.DrawLatex(0.69, 0.70, "#chi^{2}_{red} = %.1f" %  (fGauss.GetChisquare() / fGauss.GetNDF()))

tex.DrawLatex(0.69, 0.62, "Breit-Wigner Fit")
tex.DrawLatex(0.69, 0.57, "M_{Z} = %.3f #pm %.3f" %  (fBw.GetParameter(1), fBw.GetParError(1)))
tex.DrawLatex(0.69, 0.52, "#Gamma = %.3f #pm %.3f" % (fBw.GetParameter(2), fBw.GetParError(2)))
tex.DrawLatex(0.69, 0.47, "#chi^{2}_{red} = %.1f" %  (fBw.GetChisquare() / fBw.GetNDF()))

tex.DrawLatex(0.69, 0.39, "Convoluted Fit")
tex.DrawLatex(0.69, 0.34, "M_{Z} = %.3f #pm %.3f" %  (fConv.GetParameter(1), fConv.GetParError(1)))
tex.DrawLatex(0.69, 0.29, "#Gamma = %.3f #pm %.3f" % (fConv.GetParameter(2), fConv.GetParError(2)))
tex.DrawLatex(0.69, 0.24, "#sigma = %.3f #pm %.3f" % (fConv.GetParameter(5), fConv.GetParError(5)))
tex.DrawLatex(0.69, 0.19, "#chi^{2}_{red} = %.1f" %  (fConv.GetChisquare() / fConv.GetNDF()))
