...
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}  c c c}
	\toprule
	Gas & Symbol & Mischungsverhältnis \\
	\midrule
...
\begin{center}
    ...
    \footnotetext{\url{https://de.wikipedia.org/wiki/Erdatmosphäre#/media/Datei:Atmosphäre_Stufen.svg}}
\end{center}
...
\textit{Differential Optical Absorption Spectroscopy} (DOAS) wird verwendet um die Säulendichte eines bestimmten Spurengases in der Atmosphäre zu bestimmen.
...
kann die charakteristische Absorption der Gase daran erkannt werden, dass die Intensitäten an manchen stellen kleiner sind.

Wenn man nun weiß welche schmalbandigen Absorptionsstrukturen zu welchen Gasen gehören kann die Säulendichte des Gases in der Atmosphäre bestimmt werden.\\
...
Daher ist hier die Absorption durch Spurengase viel geringer, sodass man eine ähnliche Differenz wie bei einer Messung mit Satellit erhält.
...
Den Einfluss von Kurz- und Breitband Effekten auf ein unbeeinflusstes Spektrum $I_0$ kann man in Abbildung \ref{fig:rayleigh_mie} beobachten.
...
minimiert wird. 
Durch hinzufügen von $I_\text{ref}$ kann ungewollte Strahlung im Spektrometer, welche man als Streulch bezeichnet, berücksichtigt werden.
Dies sollte vor allem dann verwendet werden, wenn die Streulichtsintensität pber das Spektrum variiert und somit die optische Dichte beeinflusst.
...
Das Wirkungsquerschnitt von \ch{H2O} wurde, im Gegensatz zu den anderen Spektren, vergrößert. 
...
Der optimale Fitbereich lässt sich über eine Überlagerung (siehe Abbildung \ref{fig:range_search}) des aufgenommenen Spektrums mit dem \ch{NO2} Wirkungsquerschnitt finden.
...
Das Wirkungsquerschnitt von \ch{H2O} kann bei \ch{O3} nicht verwendet werden, da es in diesen Bereichen Singularitäten aufweist.
...
Grund dafür sind chemische Prozesse welche unterschiedlich stark bei verschiedener Sonneneinstrahlungen stattfinden.
...
Unsere Messdaten zeigen bei \ch{O3} als auch bei \ch{NO2} eine konstante schräge Säulendichte über den Tag.
...
Am Tag der Messung war es sehr bewölkt, so dass man davon ausgehen kann dass $\Delta$SCD von \ch{O4} sehr stark von der Bewölkung abhängt.
