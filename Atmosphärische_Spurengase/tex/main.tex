...
Due to a lack of clear sky conditions during this week, the analyzed data was from the $23^\text{th}$ april, 2019. One calculated a vertical column density of $\sim 300 \si{DU}$ for \ch{formula}, which is an underestimation when compared with the data NASA provided (measured $\sim 350 \si{DU}$).\\ 
The measurements of the tropospheric concentrations was not very purposful because heavy and thick clouds interfered with the measurements. Regardless of this setback one found at least, with a comparison of the vertical densities from the troposhere and starosphere, that \ch{O3} is mainly found in the stratosphere. 
...
Der Referenzwirkungsquerschnitt von \ch{H2O} kann bei \textcolor{red}{\ch{O3} ..} nicht verwendet werden, da es in diesen Bereichen Singularitäten aufweist.
...
\Delta\text{SCD} =& \text{SCD}_{\text{Mess}} - \text{SCD}_\text{ref} \\
    ...
    =& \text{SCD}_{\text{Trop, \ Mess}} - \text{SCD}_{\text{Trop, \ Ref}}.
\end{align}

Bei niedrigen Winkeln sind die Lichtstrahlen statistisch länger in der Troposphäre und werden daher stärker absorbiert.
Durch berechnen des Verhältnisses der $\Delta$SCD Werte lässt sich ein Vergleich zwischen den einzelnen Molekülen ziehen,
denn diese geben an bei welchen Molekülen am meisten in der Troposphäre absorbiert wird.
\begin{center}
...
Den niedrigsten Wert hatte hier \ch{O3}, weshalb zu vermuten ist, dass es Hauptsächlich in der Stratosphäre vorzufinden ist.
\ch{O4} hatte die stärkste Veränderung. 
