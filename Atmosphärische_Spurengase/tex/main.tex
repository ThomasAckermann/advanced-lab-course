...
The main goal was to determine the column density (CD) of \ch{NO2} and \ch{O3} in the atmosphere by using the \textit{Differential Optical Absorption Spectroscopy} (DOAS) method. \\
...
Measurements were taken over the day with the telescope adjusted to $0^\circ$ solar zenith angle (SZA). With the recorded differential slant column densities ($\Delta SCD$) and the approximation of the air mass factor (AMF) one obtained the total slant column density (SCD) through a Langley plot. Furthermore one calculated the vertical column density of \ch{O3}, which resulted in $\sim 300 \si{DU}$.\\ 
The tropospheric and stratospheric profile of \ch{O3}, \ch{O4} and \ch{NO2} could be analyzed by taking measurements at telescope angles of $7^\circ$ and $12^\circ$.
The sensitivities of the differential slant column densities ($\Delta SCD$) of the trace gases to the change in troposheric light path was quantified by the ratio $r = \Delta SCD(7^\circ)/\Delta SCD_(12^\circ)$. The measurements were not very descriptive because heavy and thick clouds decreased the validity of the taken approximation.
Nevertheless a trend for the three trace gases \ch{O3}, \ch{No2} and \ch{O4} were observed.  
The ratio between the measurments at $7^{\circ}$ and $12^{\circ}$ yields that \ch{O3} is mainly found in the stratosphere where the other two are also present in the troposhere.
...
\begin{center}
    ...
    \captionof{figure}{Atmosphärische Schichten \cite{amt_zones}}
\end{center}
...
Ebenso ist das Mischverhältnis eine wichtige Größe.
...
\begin{center}
    ...
    \captionof{figure}{Fraunhoferspektrum \cite{fraunhofer}}	
...
\begin{center}
    ...
    \captionof{figure}{Darstellung von Kurz- und Breitbandeffekten\cite{platt10}}	
...
Dies sollte vor allem dann verwendet werden, wenn die Streulichtintensität über das Spektrum variiert und somit die optische Dichte beeinflusst.

\subsection{Chemische Prozesse in der Atmosphäre}\label{ssec:chemie_atmosphäre}

Bei \ch{NO2} handelt es sich um ein Spurengas, das nicht konstant über den Tag vorhanden ist.
Am Morgen nimmt die Sonneneinstrahlung zu, wodurch die Photolyse von \ch{NO2}
\begin{align}
    \ch{NO2} + h \nu \to \ch{NO} + \ch{O} (^3\text{P}), \label{eq:creation_no}
\end{align}
vermehrt stattfindet.
Dadurch sinkt das Mischverhältnis von \ch{NO2} zunächst.
...
\begin{align}
    ...
    \ch{NO3} + h\nu \to \ch{NO2} + \ch{O}.
\end{align}
Diese Reaktionen treten vermehrt über den Tag auf, wodurch das \ch{NO2} Vorkommen wieder steigt. 
Dies lässt sich im Versuch beobachten.\\

Des Weiteren ist das \ch{O3} Vorkommen über die Gleichgewichtsreaktion
\begin{align}
    \ch{O3} + \ch{NO} \rightleftharpoons \ch{NO2} + \ch{O2},
\end{align}
mit dem \ch{NO2} Vorkommen verknüpft.
Aus der Reaktion (\ref{eq:creation_no}) geht hervor, dass sich über den Tag das \ch{O3} Vorkommen leicht verringert, da mehr \ch{NO} entsteht.

\subsection{Verteilung von \ch{NO2}, \ch{O3} und \ch{O4}}

Abbildung \ref{fig:o3_distribution} zeigt den Verlauf der Verteilung von \ch{O3} in Tropo- und Stratosphäre.
Es ist zu erkennen, dass das Ozon mit Abstand am meisten in der Stratosphäre vorkommt.

\begin{center}
    \includegraphics[width=\linewidth]{fig/o3_distribution.png}
    \captionof{figure}{Verteilung von \ch{O3} in der Atmosphäre \cite{atm_script}}
    \label{fig:o3_distribution}
\end{center}

\ch{NO2} zeigt ein sehr ähnliches Verhalten.
Jedoch ist die Verteilung in der Troposphäre stark von anthropogenen Einflüssen abhängig.

\ch{O4} ist ein Sauerstoffdimer, welches bei der Kollision von \ch{O2} entsteht.
Da die Dichte der Atmosphäre mit zunehmender Höhe exponentiell abnimmt, das Mischverhältnis von \ch{O2} konstant bleibt, nimmt auch die Menge des \ch{O4} exponentiell ab.
...
Bei diesem handelt es sich um ein elektrisches Signal, was auf jedes durch Photonen generierte Signal addiert wird. Dies ist nötig, da der verwendete AD-Wandler keine negativen Eingangssignale, welche durch Rauschminima entstehen könnten, umwandeln kann.
...
\begin{center}
    ...
    \captionof{figure}{Abhängigkeit des Dunkelstromes von der Messdauer}
...
Durch Schwankungen der Halogenlampenintensität entsteht in den Daten die beobachtete Krümmung, welche bei höherer Scanzahl größer wird. Die Schwankungen verlieren bei höheren Counts an Bedeutung.
...
Es wurden Absorptionswirkungsquerschnitte von  \ch{NO2}, \ch{H2O}, \ch{O3} und \ch{O4} angepasst.
Der Absorptionswirkungsquerschnitt von \ch{H2O} wurde, im Gegensatz zu den anderen Spektren, vergrößert. 
...
Änderung dieser Parameter sorgen für eine zusätzliche Breitbandstruktur, die durch das Polynom $\sum_k b_k \lambda^k$ in Gleichung \ref{eq:chi_squared} berücksichtigt wird.\\
...
Der optimale Fitbereich lässt sich über eine Überlagerung (siehe Abbildung \ref{fig:range_search}) des aufgenommenen Spektrums mit dem \ch{NO2} Absorptionswirkungsquerschnitt finden.
...
Für die genau Bestimmung der \ch{NO2} Konzentration wurde ein Wellenlängenbereich von $413.46 - 477.25$ \si{nm} ausgewählt.
...
erreicht werden.
Mit der nun bestimmten Säulendichte kann nun die \ch{NO2} Konzentration in der Küvette berechnet werden.
...
Der vorliegende Absorptionswirkungsquerschnitt von \ch{H2O} kann bei \ch{O3} nicht verwendet werden, da es in diesen Bereichen Singularitäten aufweist.
...
\begin{center}
    ...
    \captionof{figure}{$\Delta \text{SCD}$ von \ch{NO2} über den Tag des 23.04.2019}
	\label{fig:delta_SCD_time_NO2}
\end{center}
       
\begin{center}
	\includegraphics[width=\linewidth]{fig/SCD_Time_Plot_O3.pdf}
    \captionof{figure}{$\Delta \text{SCD}$ von \ch{O3} über den Tag des 23.04.2019}
...
Bei \ch{O3} ist kein Einfluss durch den Feierabendverkehr wie in Abbildung \ref{fig:delta_SCD_time_NO2} zu erkennen, da \ch{O3} viel stärker in der Stratosphäre als in der Troposphäre vorkommt, sodass kleine \ch{O3} Änderungen nur im Fehlerbereich liegen.


Eine Untersuchung der SCD gegen den \textit{Luftmassenfaktor}(AMF), lässt Rückschlüsse über eine Säulendichteänderung über den Tag zu.
...
Unsere Messdaten zeigen bei \ch{O3} als auch bei \ch{NO2} im Langley-Plot eine konstante schräge Säulendichte über den Tag.
Allerdings muss beachtet werden, dass Messungen vom Morgen nicht vorhanden sind.
Die schräge Säulendichte von \ch{NO2} ist, wie in Abschnitt \ref{sssec:tagesmessung} besprochen, am Morgen geringer als am Abend.
...
Der horizontale Verlauf der \textit{vertikalen Säulendichte} (VCD) bestätigt die Aussage, dass die \ch{O3} Säulendichte konstant über den Tag ist.
...
\begin{center}
    ...
    \captionof{figure}{DOAS VCD Messung mit Satelit aufgenommen \cite{giovanni_nasa}}
	\label{fig:DOAS_satelit}
\end{center} 

\begin{center}
	\includegraphics[width=\linewidth]{fig/TOMS_VCD_Ozone_ohne_Maus.png}
    \captionof{figure}{TOMS VCD Messung mit Satelit aufgenommen \cite{giovanni_nasa}}
...
Die Messung wurde bei starker Bewölkung von 14:41 bis 15:01 MESZ durchgeführt (siehe Abbildung \ref{fig:weather}).
...
Der Fit von \ch{O4} wurde im Wellenlängenbereich von $461.76 - 489.80$\si{nm} durchgeführt, wobei dieser Bereich bei einigen Messungen angepasst werden musste. 
...
Es wurden somit $VCD_{trop}$ Werte im Bereich von $3.3 - 7.7 \si{DU}$ gemessen. Dadurch lässt sich mit dem Vergleich zur stratosphärischen Messung von $\sim 300 \si{DU}$ (vgl. Abbildung \ref{fig:VCD_O3_over_day}) festhalten, dass \ch{O3} vorwiegend in der Stratosphäre vorkommt. Dieses Ergebnis liefert nur einen ersten Anhaltspunkt, da starke Vereinfachungen getroffen wurden.\\
...
Am Tag der Messung war es sehr bewölkt, sodass man davon ausgehen kann dass $\Delta$SCD von \ch{O4} sehr stark von der Bewölkung abhängt.
Eine abschließende und genau Beurteilung der Messergebnisse ließen die Wetterverhältnisse nicht zu, da eine dichte Wolkendecke die Messergebnisse stark beeinflusste, die Messdaten legen nur die Vermutung nahe, dass \ch{O4} und \ch{NO2} auch vermehrt in der Troposphäre zu finden sind.
...
Allerdings lässt sich die Tendenz erahnen, dass \ch{O3} vor allem in der Stratosphäre auftritt. Für \ch{NO2} und \ch{O4} lässt sich eine höhere Abhängigkeit von dem Teleskopwinkel beobachten, sodass der Schluss nahe liegt, dass diese auch vermehrt in der Troposphäre zu finden sind. Dies deckt sich mit dem bekannten Vorkommen.
