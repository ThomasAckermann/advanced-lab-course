...
def make_hist(bool_list, bool_events):	
	...
	for jentry in range(0, nEntries):
		...
		if bool_list[10] and bool_events[jentry] == True:
			...
			mass_z = math.sqrt(abs(2 * l1 * l2))/1000
...
mass_cz, mass_lz = [1]*11, [1]*11
#for i in range(11):
#	mass_cz[i] = ROOT.TH1D("mass_c",   "Distribution of the mass with {} trigger; mass [GeV/c^2]; Entries".format(i), 100, 0, 120)
#	mass_lz[i] = ROOT.TH1D("mass_l",   "Distribution of the mass with {} trigger; mass [GeV/c^2]; Entries".format(i), 100, 0, 120)
cutflow_hist = ROOT.TH1D("cutflow_hist",   "Total number of events selected by the consecutive criteria; ; Entries", 12, 0, 12)
	...
	for j in range(0, nEntries):
		...
		if data[j] and i >= 4:
			...
			if mass_c > 0 and mass_l > 0:
				...
				mass_lz[i].Fill(math.sqrt(2 * mass_l)/1000, weight_list[j])

	# n_events = data.count(True)
	# cutflow_hist.Fill(n_events, 1)
	mass_cz[i].Draw()
	mass_lz[i].Draw()
	a[i] = True	
	if i >= 1:
