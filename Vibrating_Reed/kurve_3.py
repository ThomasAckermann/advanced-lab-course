...
omega = np.loadtxt('data/data_3.txt', skiprows=1, unpack=True)[0]
amplitude = np.loadtxt('data/data_3.txt', skiprows=1, unpack=True)[1]
# Fehler Daten laden
amplitude_err = np.loadtxt('data/data_3.txt', skiprows=1, unpack=True)[5]

# Fit Parameter
popt_lorentz, pcov_lorentz = curve_fit(lorentz, omega[70:], amplitude[70:], p0=[144, 20, 20])# , sigma=amplitude_err)
...
plt.title('Schwingungsamplitude mit Temperatur von: 27.2°C')

plt.tight_layout()
# f_gcf = plt.gcf()
# f_gcf.set_size_inches(8.27, 11.69)
plt.savefig('fig/Resonanz140_temp3.pdf')
