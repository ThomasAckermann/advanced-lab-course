...
omega = np.loadtxt('data/data_8.txt', skiprows=1, unpack=True)[0]
amplitude = np.loadtxt('data/data_8.txt', skiprows=1, unpack=True)[1]
# Fehler Daten laden
amplitude_err = np.loadtxt('data/data_8.txt', skiprows=1, unpack=True)[5]
...
plt.title('Schwingungsamplitude mit Temperatur von: 33.0°C')

plt.tight_layout()
# f_gcf = plt.gcf()
# f_gcf.set_size_inches(8.27, 11.69)
plt.savefig('fig/Resonanz140_temp8.pdf')
